...
def update_screen(ai_settings,screen,stats,scoreboard,ship,bullets,aliens,play_button):
	...
	screen.fill(ai_settings.bg_color)#给屏幕填满颜色 
	for bullet in bullets.sprites():
		bullet.draw_bullet()
	
	
	ship.blitme()#调用了ship实例的传图方法
	
	aliens.draw(screen)
	if not stats.game_active:
		play_button.draw_button()
	scoreboard.show_score()
	pygame.display.flip()#将刚刚渲染的图片显示在屏幕上 
...
def creat_alien(ai_settings,screen,aliens,alien_number,row_nubmer):
...
def creat_fleet(ai_settings,screen,ship,aliens):
	...
	for row_nubmer in range(number_rows):
		for alien_number in range(number_aliens_x):
			creat_alien(ai_settings,screen,aliens,alien_number,row_nubmer)
...
def change_fleet_direction(ai_setting,aliens):
	...
	ai_setting.fleet_direction= ai_setting.fleet_direction * -1 
